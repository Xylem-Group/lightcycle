...
- **Event log decoding**: the two universal TRC-20 events (`Transfer` / `Approval`) are recognized by topic-0 hash and ship in v0.1 — they cover every TRC-20 token without any registry. Arbitrary-contract event decoding (TRC-721, custom events) requires an ABI registry and lands behind a future `decode_event(log, abi)` entry point. ABI registry shape is pluggable: file-based, HTTP, or on-chain via `getContract`.
